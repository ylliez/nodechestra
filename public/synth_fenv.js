console.log(`nodechestra filter envelope page loaded`);

const socket = io("/fenv");
socket.on("connect", () => {
    ...
    if (handsOn) {
        for (let i = 0; i < handsOn; i++) {
            ...
            if (results.multiHandedness[i].label === `Left`) {
                ...
                canvasCtx.fill();
                socket.emit("fenv", `fenv fenvA ${indexTip.x * 1000}`);
                socket.emit("fenv", `fenv fenvD ${(1 - indexTip.y) * 1000}`);
            }
            if (results.multiHandedness[i].label === `Right`) {
                ...
                canvasCtx.fill();
                socket.emit("fenv", `fenv fenvS ${1 - indexTip.y}`);
                socket.emit("fenv", `fenv fenvR ${indexTip.x * 1000}`);
            }
            canvasCtx.restore();
        }
    }
