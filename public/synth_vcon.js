console.log(`nodechestra voice contralto page loaded`);

const socket = io("/vcon");
socket.on("connect", () => {
        ...
        if (results.multiFaceLandmarks[0]) {
            ...
            if (lipAp > 0.01) {
                if (voiceMIDI != voiceMIDIEx) {
                    socket.emit("voice", `voice 4 ${voiceMIDIEx} 0`);
                    voiceMIDIEx = voiceMIDI
                    socket.emit("voice", `voice 4 ${voiceMIDI} 127`);
                }
            }
            else {
                socket.emit("voice", `voice 4 ${voiceMIDI} 0`);
                voiceMIDIEx = 0
            }
        }
        canvasCtx.restore();
